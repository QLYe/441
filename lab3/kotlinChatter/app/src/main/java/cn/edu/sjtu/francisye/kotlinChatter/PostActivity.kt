...
import cn.edu.sjtu.francisye.kotlinChatter.ChattStore.postChatt
import cn.edu.sjtu.francisye.kotlinChatter.databinding.ActivityPostBinding
...
class PostActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        view.cameraButton.setOnClickListener {
            viewState.imageUri = mediaStoreAlloc("image/jpeg")
            takePictureLauncher.launch(viewState.imageUri)
        }

        val captureVideoLauncher = registerForActivityResult(ActivityResultContracts.CaptureVideo()) { uri ->
            viewState.videoIcon = android.R.drawable.presence_video_busy
            view.videoButton.setImageResource(viewState.videoIcon)
        }
        view.videoButton.setOnClickListener {
            viewState.videoUri = mediaStoreAlloc("video/mp4")
            captureVideoLauncher.launch(viewState.videoUri)
        }
